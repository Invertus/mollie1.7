...
class PaymentMethodConfigurationUpdater
{
	...
	public function updatePaymentMethodConfiguration(array $paymentMethodData)
	{
		if (empty($paymentMethodData)) {
			throw new PaymentMethodConfigurationUpdaterException('Failed to update payment method configuration. Payment method (%s) configuration was not saved to database', PaymentMethodConfigurationUpdaterException::NO_PAYMENT_METHOD_DATA_PROVIDED, $paymentMethodData['name']);
		}

		try {
			$paymentMethod = $this->paymentMethodService->savePaymentMethod($paymentMethodData);
		} catch (Exception $e) {
			throw new PaymentMethodConfigurationUpdaterException('Failed to update payment method configuration. Payment method (%s) configuration was not saved to database', PaymentMethodConfigurationUpdaterException::FAILED_TO_SAVE_PAYMENT_METHOD, $paymentMethodData['name']);
		}

		if (!$this->paymentMethodRepository->deletePaymentMethodIssuersByPaymentMethodId($paymentMethod->id)) {
			throw new PaymentMethodConfigurationUpdaterException('Failed to update payment method configuration. Payment methods (%s) old issuers unable to be deleted', PaymentMethodConfigurationUpdaterException::FAILED_TO_DELETE_OLD_ISSUERS, $paymentMethodData['name']);
		}
		...
		if (isset($paymentMethodData['issuers'])) {
			...
			try {
				...
				throw new PaymentMethodConfigurationUpdaterException('Failed to update payment method configuration. Payment methods (%s) issuers unable to be saved', PaymentMethodConfigurationUpdaterException::FAILED_TO_SAVE_ISSUERS, $paymentMethodData['name']);
			}
		}
