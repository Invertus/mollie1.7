...
use MolliePrefix\Doctrine\Instantiator\Exception\InvalidArgumentException as InstantiatorInvalidArgumentException;
use MolliePrefix\Doctrine\Instantiator\Exception\UnexpectedValueException as InstantiatorUnexpectedValueException;
/**
...
class PHPUnit_Framework_MockObject_Generator
{
     ...
     * @var array
     */
    private $blacklistedMethodNames = ['__CLASS__' => \true, '__DIR__' => \true, '__FILE__' => \true, '__FUNCTION__' => \true, '__LINE__' => \true, '__METHOD__' => \true, '__NAMESPACE__' => \true, '__TRAIT__' => \true, '__clone' => \true, '__halt_compiler' => \true];
    /**
     ...
     */
    public function getMock($type, $methods = [], array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $cloneArguments = \true, $callOriginalMethods = \false, $proxyTarget = null, $allowMockingUnknownTypes = \true)
    {
        if (!\is_array($type) && !\is_string($type)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(1, 'array or string');
        }
        if (!\is_string($mockClassName)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(4, 'string');
        }
        if (!\is_array($methods) && !\is_null($methods)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(2, 'array', $methods);
        }
            ...
            if (\is_array($type)) {
                foreach ($type as $_type) {
                    if (!\class_exists($_type, $callAutoload) && !\interface_exists($_type, $callAutoload)) {
                        throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Cannot stub or mock class or interface "%s" which does not exist', $_type));
                    }
                }
            } else {
                if (!\class_exists($type, $callAutoload) && !\interface_exists($type, $callAutoload)) {
                    throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Cannot stub or mock class or interface "%s" which does not exist', $type));
                }
            }
        }
        if (null !== $methods) {
            foreach ($methods as $method) {
                if (!\preg_match('~[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*~', $method)) {
                    throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Cannot stub or mock method with invalid name "%s"', $method));
                }
            }
            if ($methods != \array_unique($methods)) {
                throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Cannot stub or mock using a method list that contains duplicates: "%s" (duplicate: "%s")', \implode(', ', $methods), \implode(', ', \array_unique(\array_diff_assoc($methods, \array_unique($methods))))));
            }
        }
        if ($mockClassName != '' && \class_exists($mockClassName, \false)) {
            $reflect = new \ReflectionClass($mockClassName);
            if (!$reflect->implementsInterface('PHPUnit_Framework_MockObject_MockObject')) {
                throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Class "%s" already exists.', $mockClassName));
            }
        }
        if ($callOriginalConstructor === \false && $callOriginalMethods === \true) {
            throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException('Proxying to original methods requires invoking the original constructor');
        }
    ...
    private function getObject($code, $className, $type = '', $callOriginalConstructor = \false, $callAutoload = \false, array $arguments = [], $callOriginalMethods = \false, $proxyTarget = null)
    {
        ...
        if ($callOriginalConstructor && \is_string($type) && !\interface_exists($type, $callAutoload)) {
            ...
            try {
                $instantiator = new \MolliePrefix\Doctrine\Instantiator\Instantiator();
                $object = $instantiator->instantiate($className);
            } catch (\MolliePrefix\Doctrine\Instantiator\Exception\UnexpectedValueException $exception) {
                if ($exception->getPrevious()) {
                    $exception = $exception->getPrevious();
                }
                throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException($exception->getMessage());
            } catch (\MolliePrefix\Doctrine\Instantiator\Exception\InvalidArgumentException $exception) {
                throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException($exception->getMessage());
            }
        }
     ...
     * @since  Method available since Release 1.0.0
     */
    public function getMockForAbstractClass($originalClassName, array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $mockedMethods = [], $cloneArguments = \true)
    {
        if (!\is_string($originalClassName)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(1, 'string');
        }
        if (!\is_string($mockClassName)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(3, 'string');
        }
        if (\class_exists($originalClassName, $callAutoload) || \interface_exists($originalClassName, $callAutoload)) {
            ...
            throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Class "%s" does not exist.', $originalClassName));
        }
    }
    /**
     ...
     */
    public function getMockForTrait($traitName, array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $mockedMethods = [], $cloneArguments = \true)
    {
        ...
        if (!\trait_exists($traitName, $callAutoload)) {
            throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Trait "%s" does not exist.', $traitName));
        }
     ...
     */
    public function getObjectForTrait($traitName, array $arguments = [], $traitClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true)
    {
        if (!\is_string($traitName)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(1, 'string');
        }
     ...
     * @return array
     */
    public function generate($type, array $methods = null, $mockClassName = '', $callOriginalClone = \true, $callAutoload = \true, $cloneArguments = \true, $callOriginalMethods = \false)
    {
        if (\is_array($type)) {
            \sort($type);
        }
    ...
    private function generateMock($type, $methods, $mockClassName, $callOriginalClone, $callAutoload, $cloneArguments, $callOriginalMethods)
    {
        ...
        if (\is_array($type)) {
            foreach ($type as $_type) {
                if (!\interface_exists($_type, $callAutoload)) {
                    throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Interface "%s" does not exist.', $_type));
                }
                $isMultipleInterfaces = \true;
                ...
                foreach ($this->getClassMethods($_type) as $method) {
                    if (\in_array($method, $methods)) {
                        throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Duplicate method "%s" not allowed.', $method));
                    }
        ...
        if (!$isClass && !$isInterface) {
            ...
            if ($class->isFinal()) {
                throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Class "%s" is declared "final" and cannot be mocked.', $mockClassName['fullClassName']));
            }
        ...
        if (isset($class)) {
            ...
            if ($isInterface && $class->implementsInterface('Traversable') && !$class->implementsInterface('Iterator') && !$class->implementsInterface('IteratorAggregate')) {
            ...
            foreach ($methods as $methodName) {
                try {
                    ...
                    if ($this->canMockMethod($method)) {
                        $mockedMethods .= $this->generateMockedMethodDefinitionFromExisting($templateDir, $method, $cloneArguments, $callOriginalMethods);
                    }
                } catch (\ReflectionException $e) {
                    $mockedMethods .= $this->generateMockedMethodDefinition($templateDir, $mockClassName['fullClassName'], $methodName, $cloneArguments);
                }
            }
        } elseif ($isMultipleInterfaces) {
            foreach ($methods as $methodName) {
                if ($this->canMockMethod($methodReflections[$methodName])) {
                    $mockedMethods .= $this->generateMockedMethodDefinitionFromExisting($templateDir, $methodReflections[$methodName], $cloneArguments, $callOriginalMethods);
                }
            }
        } else {
            foreach ($methods as $methodName) {
        ...
        return ['code' => $classTemplate->render(), 'mockClassName' => $mockClassName['className']];
    }
    /**
     * @param array|string $type
     * @param string       $className
    ...
    private function generateMockedMethodDefinitionFromExisting($templateDir, \ReflectionMethod $method, $cloneArguments, $callOriginalMethods)
    {
        ...
        return $this->generateMockedMethodDefinition($templateDir, $method->getDeclaringClass()->getName(), $method->getName(), $cloneArguments, $modifier, $this->getMethodParameters($method), $this->getMethodParameters($method, \true), $returnType, $reference, $callOriginalMethods, $method->isStatic(), $deprecation, $this->allowsReturnNull($method));
    }
    /**
     * @param string       $templateDir
     * @param string       $className
     ...
     * @return string
     */
    private function generateMockedMethodDefinition($templateDir, $className, $methodName, $cloneArguments = \true, $modifier = 'public', $arguments_decl = '', $arguments_call = '', $return_type = '', $reference = '', $callOriginalMethods = \false, $static = \false, $deprecation = \false, $allowsReturnNull = \false)
    {
        if ($static) {
            ...
            if (!$forCall) {
                if ($this->hasType($parameter) && (string) $parameter->getType() !== 'self') {
                    ...
                    try {
                        ...
                        throw new \MolliePrefix\PHPUnit_Framework_MockObject_RuntimeException(\sprintf('Cannot mock %s::%s() because a class or ' . 'interface used in the signature is not loaded', $method->getDeclaringClass()->getName(), $method->getName()), 0, $e);
                    }
                    if ($class !== null) {
    ...
    private function getTemplate($filename)
    {
        if (!isset(self::$templates[$filename])) {
            self::$templates[$filename] = new \MolliePrefix\Text_Template($filename);
        }
