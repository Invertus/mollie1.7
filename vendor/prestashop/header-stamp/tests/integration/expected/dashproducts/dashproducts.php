...
class dashproducts extends \test\Module
{
    ...
    {
        \test\Configuration::updateValue('DASHPRODUCT_NBR_SHOW_LAST_ORDER', 10);
        \test\Configuration::updateValue('DASHPRODUCT_NBR_SHOW_BEST_SELLER', 10);
        \test\Configuration::updateValue('DASHPRODUCT_NBR_SHOW_MOST_VIEWED', 10);
        \test\Configuration::updateValue('DASHPRODUCT_NBR_SHOW_TOP_SEARCH', 10);
    ...
    {
        $this->context->smarty->assign(array('DASHACTIVITY_CART_ACTIVE' => \test\Configuration::get('DASHACTIVITY_CART_ACTIVE'), 'DASHACTIVITY_VISITOR_ONLINE' => \test\Configuration::get('DASHACTIVITY_VISITOR_ONLINE'), 'DASHPRODUCT_NBR_SHOW_LAST_ORDER' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_LAST_ORDER'), 'DASHPRODUCT_NBR_SHOW_BEST_SELLER' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_BEST_SELLER'), 'DASHPRODUCT_NBR_SHOW_TOP_SEARCH' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_TOP_SEARCH'), 'date_from' => \test\Tools::displayDate($params['date_from']), 'date_to' => \test\Tools::displayDate($params['date_to']), 'dashproducts_config_form' => $this->renderConfigForm()));
    ...
    public function getTableRecentOrders()
    {
        ...
        $limit = (int) \test\Configuration::get('DASHPRODUCT_NBR_SHOW_LAST_ORDER') ? (int) \test\Configuration::get('DASHPRODUCT_NBR_SHOW_LAST_ORDER') : 10;
        $orders = \test\Order::getOrdersWithInformations($limit);
        $body = array();
        foreach ($orders as $order) {
            $currency = \test\Currency::getCurrency((int) $order['id_currency']);
            $tr = array();
            $tr[] = array('id' => 'firstname_lastname', 'value' => '<a href="' . $this->context->link->getAdminLink('AdminCustomers', \true) . '&id_customer=' . $order['id_customer'] . '&viewcustomer">' . \test\Tools::htmlentitiesUTF8($order['firstname']) . ' ' . \test\Tools::htmlentitiesUTF8($order['lastname']) . '</a>', 'class' => 'text-left');
            $tr[] = array('id' => 'total_products', 'value' => \count(\test\OrderDetail::getList((int) $order['id_order'])), 'class' => 'text-center');
            $tr[] = array('id' => 'total_paid', 'value' => \test\Tools::displayPrice((float) $order['total_paid_tax_excl'], $currency), 'class' => 'text-center', 'wrapper_start' => $order['valid'] ? '<span class="badge badge-success">' : '', 'wrapper_end' => '<span>');
            $tr[] = array('id' => 'date_add', 'value' => \test\Tools::displayDate($order['date_add']), 'class' => 'text-center');
            $tr[] = array('id' => 'status', 'value' => \test\Tools::htmlentitiesUTF8($order['state_name']), 'class' => 'text-center');
            $tr[] = array('id' => 'details', 'value' => '', 'class' => 'text-right', 'wrapper_start' => '<a class="btn btn-default" href="index.php?tab=AdminOrders&id_order=' . (int) $order['id_order'] . '&vieworder&token=' . \test\Tools::getAdminTokenLite('AdminOrders') . '" title="' . $this->trans('Details', array(), 'Modules.Dashproducts.Admin') . '"><i class="icon-search"></i>', 'wrapper_end' => '</a>');
            $body[] = $tr;
        }
    ...
    public function getTableBestSellers($date_from, $date_to)
    {
        ...
        $products = \test\Db::getInstance()->ExecuteS('
					SELECT
						product_id,
						product_name,
		...
		WHERE `invoice_date` BETWEEN "' . \test\pSQL($date_from) . ' 00:00:00" AND "' . \test\pSQL($date_to) . ' 23:59:59"
		AND valid = 1
		' . \test\Shop::addSqlRestriction(\false, 'o') . '
		GROUP BY product_id, product_attribute_id
		ORDER BY total DESC
		LIMIT ' . (int) \test\Configuration::get('DASHPRODUCT_NBR_SHOW_BEST_SELLER'));
        $body = array();
        foreach ($products as $product) {
            $product_obj = new \test\Product((int) $product['product_id'], \false, $this->context->language->id);
            if (!\test\Validate::isLoadedObject($product_obj)) {
                continue;
            }
            $category = new \test\Category($product_obj->getDefaultCategory(), $this->context->language->id);
            $img = '';
            if (($row_image = \test\Product::getCover($product_obj->id)) && $row_image['id_image']) {
                $image = new \test\Image($row_image['id_image']);
                $path_to_image = \_PS_PROD_IMG_DIR_ . $image->getExistingImgPath() . '.' . $this->context->controller->imageType;
                $img = \test\ImageManager::thumbnail($path_to_image, 'product_mini_' . $row_image['id_image'] . '.' . $this->context->controller->imageType, 45, $this->context->controller->imageType);
            }
            $productPrice = $product['price'];
            if (isset($product['price_attribute']) && $product['price_attribute'] != '0.000000') {
                $productPrice = $product['price_attribute'];
            }
            $body[] = array(array('id' => 'product', 'value' => $img, 'class' => 'text-center'), array('id' => 'product', 'value' => '<a href="' . $this->context->link->getAdminLink('AdminProducts', \true) . '&id_product=' . $product_obj->id . '&updateproduct">' . \test\Tools::htmlentitiesUTF8($product['product_name']) . '</a>' . '<br/>' . \test\Tools::displayPrice($productPrice), 'class' => 'text-center'), array('id' => 'category', 'value' => $category->name, 'class' => 'text-center'), array('id' => 'total_sold', 'value' => $product['total'], 'class' => 'text-center'), array('id' => 'sales', 'value' => \test\Tools::displayPrice($product['sales']), 'class' => 'text-center'), array('id' => 'net_profit', 'value' => \test\Tools::displayPrice($product['sales'] - $product['expenses']), 'class' => 'text-center'));
        }
        return array('header' => $header, 'body' => $body);
    }
    public function getTableMostViewed($date_from, $date_to)
    {
        ...
        if (\test\Configuration::get('PS_STATSDATA_PAGESVIEWS')) {
            $products = $this->getTotalViewed($date_from, $date_to, (int) \test\Configuration::get('DASHPRODUCT_NBR_SHOW_MOST_VIEWED'));
            $body = array();
            if (\is_array($products) && \count($products)) {
                foreach ($products as $product) {
                    $product_obj = new \test\Product((int) $product['id_object'], \true, $this->context->language->id);
                    if (!\test\Validate::isLoadedObject($product_obj)) {
                        continue;
                    }
                    $img = '';
                    if (($row_image = \test\Product::getCover($product_obj->id)) && $row_image['id_image']) {
                        $image = new \test\Image($row_image['id_image']);
                        $path_to_image = \_PS_PROD_IMG_DIR_ . $image->getExistingImgPath() . '.' . $this->context->controller->imageType;
                        $img = \test\ImageManager::thumbnail($path_to_image, 'product_mini_' . $product_obj->id . '.' . $this->context->controller->imageType, 45, $this->context->controller->imageType);
                    }
                    $tr = array();
                    $tr[] = array('id' => 'product', 'value' => $img, 'class' => 'text-center');
                    $tr[] = array('id' => 'product', 'value' => \test\Tools::htmlentitiesUTF8($product_obj->name) . '<br/>' . \test\Tools::displayPrice(\test\Product::getPriceStatic((int) $product_obj->id)), 'class' => 'text-center');
    ...
    public function getTableTop10MostSearch($date_from, $date_to)
    {
        ...
        $terms = $this->getMostSearchTerms($date_from, $date_to, (int) \test\Configuration::get('DASHPRODUCT_NBR_SHOW_TOP_SEARCH'));
        $body = array();
    ...
    public function getTotalProductSales($date_from, $date_to, $id_product)
    {
        $sql = 'SELECT SUM(od.`product_quantity` * od.`product_price`) AS total
				...
				WHERE od.`product_id` = ' . (int) $id_product . '
					' . \test\Shop::addSqlRestriction(\test\Shop::SHARE_ORDER, 'o') . '
					AND o.valid = 1
					AND o.`date_add` BETWEEN "' . \test\pSQL($date_from) . '" AND "' . \test\pSQL($date_to) . '"';
        return (int) \test\Db::getInstance(\_PS_USE_SQL_SLAVE_)->getValue($sql);
    }
    public function getTotalProductAddedCart($date_from, $date_to, $id_product)
    {
        return \test\Db::getInstance(\_PS_USE_SQL_SLAVE_)->getValue('
		SELECT count(`id_product`) as count
		FROM `' . \_DB_PREFIX_ . 'cart_product` cp
		WHERE cp.`id_product` = ' . (int) $id_product . '
		' . \test\Shop::addSqlRestriction(\false, 'cp') . '
		AND cp.`date_add` BETWEEN "' . \test\pSQL($date_from) . '" AND "' . \test\pSQL($date_to) . '"');
    }
    public function getTotalProductPurchased($date_from, $date_to, $id_product)
    {
		...
		JOIN `' . \_DB_PREFIX_ . 'orders` o ON o.`id_order` = od.`id_order`
		WHERE od.`product_id` = ' . (int) $id_product . '
		' . \test\Shop::addSqlRestriction(\false, 'od') . '
		AND o.valid = 1
		AND o.`date_add` BETWEEN "' . \test\pSQL($date_from) . '" AND "' . \test\pSQL($date_to) . '"');
    }
    public function getTotalViewed($date_from, $date_to, $limit = 10)
    {
        $gapi = \test\Module::isInstalled('gapi') ? \test\Module::getInstanceByName('gapi') : \false;
        if (\test\Validate::isLoadedObject($gapi) && $gapi->isConfigured()) {
            $products = array();
            ...
            return \test\Db::getInstance(\_PS_USE_SQL_SLAVE_)->executeS('
			...
			' . \test\Shop::addSqlRestriction(\false, 'pv') . '
			AND dr.`time_start` BETWEEN "' . \test\pSQL($date_from) . '" AND "' . \test\pSQL($date_to) . '"
			AND dr.`time_end` BETWEEN "' . \test\pSQL($date_from) . '" AND "' . \test\pSQL($date_to) . '"
    ...
    {
        if (!\test\Module::isInstalled('statssearch')) {
            return array();
        }
        return \test\Db::getInstance(\_PS_USE_SQL_SLAVE_)->executeS('
		SELECT `keywords`, count(`id_statssearch`) as count_keywords, `results`
		FROM `' . \_DB_PREFIX_ . 'statssearch` ss
		WHERE ss.`date_add` BETWEEN "' . \test\pSQL($date_from) . '" AND "' . \test\pSQL($date_to) . '"
		' . \test\Shop::addSqlRestriction(\false, 'ss') . '
    ...
    public function renderConfigForm()
    {
        ...
        $helper = new \test\HelperForm();
        $helper->show_toolbar = \false;
        $helper->table = $this->table;
        $lang = new \test\Language((int) \test\Configuration::get('PS_LANG_DEFAULT'));
        $helper->default_form_language = $lang->id;
        $helper->allow_employee_form_lang = \test\Configuration::get('PS_BO_ALLOW_EMPLOYEE_FORM_LANG') ? \test\Configuration::get('PS_BO_ALLOW_EMPLOYEE_FORM_LANG') : 0;
        $this->fields_form = array();
        $helper->id = (int) \test\Tools::getValue('id_carrier');
    ...
    {
        return array('DASHPRODUCT_NBR_SHOW_LAST_ORDER' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_LAST_ORDER'), 'DASHPRODUCT_NBR_SHOW_BEST_SELLER' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_BEST_SELLER'), 'DASHPRODUCT_NBR_SHOW_MOST_VIEWED' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_MOST_VIEWED'), 'DASHPRODUCT_NBR_SHOW_TOP_SEARCH' => \test\Configuration::get('DASHPRODUCT_NBR_SHOW_TOP_SEARCH'));
    }
    public function hookActionObjectOrderAddAfter($params)
    {
        \test\Tools::changeFileMTime($this->push_filename);
    }
...
\class_alias('test\\dashproducts', 'test\\dashproducts', \false);
