...
class Multiplication
{
    ...
    public function computeUsingBcMath(\test\PrestaShop\Decimal\DecimalNumber $a, \test\PrestaShop\Decimal\DecimalNumber $b)
    {
        ...
        return new \test\PrestaShop\Decimal\DecimalNumber((string) bcmul($a, $b, $precision1 + $precision2));
    }
    /**
    ...
    public function computeWithoutBcMath(\test\PrestaShop\Decimal\DecimalNumber $a, \test\PrestaShop\Decimal\DecimalNumber $b)
    {
        ...
        if ('0' === $aAsString || '0' === $bAsString) {
            return new \test\PrestaShop\Decimal\DecimalNumber('0');
        }
    ...
    private function multiplyStrings($topNumber, $bottomNumber)
    {
        ...
        $stepNumber = 0;
        $result = new \test\PrestaShop\Decimal\DecimalNumber('0');
        for ($i = $bottomNumberLength - 1; $i >= 0; $i--) {
            ...
            // add to the result
            $result = $result->plus(new \test\PrestaShop\Decimal\DecimalNumber(\strrev($partialResult)));
            $stepNumber++;
        }
